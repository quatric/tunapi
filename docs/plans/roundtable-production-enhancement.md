...
- ✅ **P0** 패키지 분해 / **P1** role 배선(roles.py, tunaLlama) / **P2** 구조화 synthesis(consensus.py, tunaLlama → SynthesisArtifact 실채움) / **P3** cross-round consensus 주입 / **P4 위생** save 실패 suppress→로깅. 전부 main, config opt-in, 라이브 안전. main 3746 통과·83.2%·ruff/ty clean.
- ⛔ **보류(정당화)**: task 04 vote aggregate, task 07 convergence/doom-loop — pass/fail verdict 기반이라 **일반 토론 부적합**(코드리뷰용). 필요 시에만.
- ☐ **남음**: task 09 패리티 — tunaDish rt 구현(feature 규모, 사용자가 tunadish 사용), Discord 영속화(중간, facade 미보유 → 별도 wiring), task 08 잔여 cosmetic dedup(_now_iso/store base, 저가치).
