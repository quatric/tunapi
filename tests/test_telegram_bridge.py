...
from tunapi import commands, plugins
from tunapi.telegram.commands.executor import _CaptureTransport, _run_engine
from tunapi.telegram.commands.file_transfer import _handle_file_get, _handle_file_put
from tunapi.telegram.commands.model import _handle_model_command
from tunapi.telegram.commands.reasoning import _handle_reasoning_command
from tunapi.telegram.commands.topics import _handle_topic_command
import tunapi.telegram.loop as telegram_loop
import tunapi.telegram.topics as telegram_topics
from tunapi.directives import parse_directives
from tunapi.telegram.api_models import Chat, File, ForumTopic, Message, Update, User
from tunapi.settings import TelegramFilesSettings, TelegramTopicsSettings
from tunapi.telegram.bridge import (
...
from tunapi.runner_bridge import ExecBridgeConfig, RunningTask
from tunapi.markdown import MarkdownPresenter
...
from tunapi.runners.mock import Return, ScriptRunner, Sleep, Wait
...
def test_telegram_presenter_progress_shows_cancel_button() -> None:
    ...
    assert reply_markup["inline_keyboard"][0][0]["callback_data"] == "tunapi:cancel"
...
def test_cancel_command_accepts_extra_text() -> None:
    ...
    assert is_cancel_command("/cancel@tunapi please") is True
...
def test_topic_title_matches_command_syntax() -> None:
    ...
    title = telegram_topics._topic_title(
        runtime=cfg.runtime,
        context=RunContext(project="tunapi", branch="master"),
    )

    assert title == "tunapi @master"

    title = telegram_topics._topic_title(
        runtime=cfg.runtime,
        context=RunContext(project="tunapi", branch=None),
    )
...
async def test_maybe_rename_topic_updates_title(tmp_path: Path) -> None:
    ...
    await store.set_context(
        123,
        77,
        RunContext(project="tunapi", branch="old"),
        topic_title="tunapi @old",
    )
    ...
    await telegram_topics._maybe_rename_topic(
        ...
        chat_id=123,
        thread_id=77,
        context=RunContext(project="tunapi", branch="new"),
    )

    bot = cast(FakeBot, cfg.bot)
    assert bot.edit_topic_calls
    assert bot.edit_topic_calls[-1]["name"] == "tunapi @new"
    snapshot = await store.get_thread(123, 77)
    assert snapshot is not None
    assert snapshot.topic_title == "tunapi @new"
...
async def test_maybe_rename_topic_skips_when_title_matches(tmp_path: Path) -> None:
    ...
    await store.set_context(
        123,
        77,
        RunContext(project="tunapi", branch="main"),
        topic_title="tunapi @main",
    )
    snapshot = await store.get_thread(123, 77)
    ...
    await telegram_topics._maybe_rename_topic(
        ...
        chat_id=123,
        thread_id=77,
        context=RunContext(project="tunapi", branch="main"),
        snapshot=snapshot,
    )
...
async def test_topic_command_recreates_stale_topic(tmp_path: Path) -> None:
    ...
    await store.set_context(
        123,
        77,
        RunContext(project="tunapi", branch="master"),
        topic_title="tunapi @master",
    )
    msg = TelegramIncomingMessage(
        transport="telegram",
        chat_id=123,
        message_id=10,
        text="/topic tunapi @master",
        reply_to_message_id=None,
        reply_to_text=None,
        sender_id=123,
    )
    ...
    assert snapshot is not None
    assert snapshot.context == RunContext(project="tunapi", branch="master")
...
async def test_run_main_loop_persists_topic_sessions_in_project_scope(
    ...
    projects = ProjectsConfig(
        ...
        default_project=None,
        chat_map={project_chat_id: "tunapi"},
    )
    runtime = TransportRuntime(
        router=_make_router(runner),
        projects=projects,
    ...
    state_path = resolve_state_path(runtime.config_path or tmp_path / "tunapi.toml")
...
async def test_run_main_loop_command_updates_chat_session_resume(
    ...
    entrypoints = [
        FakeEntryPoint(
            "run_cmd",
            "tunapi.commands.run_cmd:BACKEND",
            plugins.COMMAND_GROUP,
            loader=_Command,
        )
    ]
...
async def test_run_main_loop_command_uses_project_default_engine(
    ...
    entrypoints = [
        FakeEntryPoint(
            "use_project",
            "tunapi.commands.use_project:BACKEND",
            plugins.COMMAND_GROUP,
            loader=_Command,
        )
    ]
...
async def test_run_main_loop_command_defaults_to_chat_project(
    ...
    entrypoints = [
        FakeEntryPoint(
            "auto_ctx",
            "tunapi.commands.auto_ctx:BACKEND",
            plugins.COMMAND_GROUP,
            loader=_Command,
        )
    ]
