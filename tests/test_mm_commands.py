...
from tunapi.core.chat_command_handlers import _resolve_id
...
class TestHandleModel:
    ...
    def test_set_model(self, setup):
        ...
        assert (
            anyio.run(lambda: self.prefs.get_engine_model(CH, "claude")) == "my-model"
        )
...
class TestHandleStatus:
    ...
    def test_project_bound(self, setup):
        anyio.run(
            lambda: self.prefs.set_context(
                CH, RunContext(project="myproj", branch="feat-x")
            )
        )
        self._run()
...
class TestHandleProject:
    ...
    def test_info_bound(self, setup):
        anyio.run(
            lambda: self.prefs.set_context(
                CH, RunContext(project="backend", branch="main")
            )
        )
        self._run("info")
        assert "backend" in self.texts[0]
...
class TestHelpDispatcherConsistency:
    def test_all_help_commands_have_dispatchers(self):
        ...
        assert not undispatched, (
            f"Help lists commands without dispatchers: {undispatched}"
        )
