...
from .model import Action, ActionEvent, ResumeToken, StartedEvent, TunapiEvent
...
class ProgressTracker:
    ...
    def note_event(self, event: TunapiEvent) -> bool:
        match event:
            case StartedEvent(resume=resume, title=title):
                self.resume = resume
                # Auto-detect model from CLI-reported title.  Apply when:
