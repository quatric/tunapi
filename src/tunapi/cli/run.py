...
from ..settings import TunapiSettings, load_settings, load_settings_if_exists
...
def _load_settings_optional() -> tuple[TunapiSettings | None, Path | None]:
    try:
...
def _should_run_interactive() -> bool:
    if os.environ.get("TUNAPI_NO_INTERACTIVE"):
        return False
...
def _run_auto_router(
    *,
    ...
    load_settings_fn = cast(
        Callable[[], tuple[TunapiSettings, Path]],
    ...
    if debug:
        os.environ.setdefault("TUNAPI_LOG_FILE", "debug.log")
    ...
    try:
        ...
        # Resolve transport id(s)
        transport_ids = settings.resolve_transport_ids(override=transport_override)

        if len(transport_ids) == 1:
            # -- Single transport: existing path (backward compat) --
            tid = transport_ids[0]
            if tid == "telegram":
            ...
            # -- Multi transport: prepare all, then run in parallel --
            spec = build_runtime_spec_fn(
                settings=settings,
                config_path=config_path,
                default_engine_override=default_engine_override,
            ...
            for tid in transport_ids:
                if tid == "telegram":
                    t_config = settings.transports.telegram
                    if t_config is None:
                        raise ConfigError(
                            f"Missing [transports.telegram] in {config_path}."
                        )
                ...
                backends.append(t_backend)

            # Run all prepared backends in parallel
            async def _run_all() -> None:
                async with anyio.create_task_group() as tg:
                    for b in backends:
                        tg.start_soon(b.async_run)

            logger.info(
                "multi_transport.starting",
