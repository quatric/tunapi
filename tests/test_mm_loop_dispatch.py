...
_auto_bind_channel_project, and _resolve_upload_dir in tunapi.mattermost.loop."""
...
class TestAutoBindChannelProject:
    ...
    async def test_skips_when_channel_not_found(self):
        ...
        cfg.bot._client.get_channel = AsyncMock(return_value=None)
        await _auto_bind_channel_project("ch1", cfg)
    ...
    async def test_skips_when_channel_has_no_name(self):
        ...
        channel = MagicMock()
        channel.name = ""
        cfg.bot._client.get_channel = AsyncMock(return_value=channel)
        await _auto_bind_channel_project("ch1", cfg)
        cfg.runtime._projects.register_discovered.assert_not_called()
...
class TestRunEngine:
    ...
    @pytest.mark.anyio()
    async def test_thread_handling_with_root_id(self, sessions, chat_prefs):
        """When msg has root_id, reply_to should use thread_id=root_id."""
        cfg = _make_cfg()
        cfg.runtime.resolve_message.return_value = _make_resolved_message()
        cfg.runtime.resolve_engine.return_value = "claude"
        cfg.runtime.format_context_line.return_value = None
        ...
        assert incoming.reply_to.thread_id == "root123"
        assert incoming.thread_id == "root123"

    @pytest.mark.anyio()
    async def test_thread_handling_without_root_id(self, sessions, chat_prefs):
        """When msg has no root_id, reply_to should be None."""
        cfg = _make_cfg()
        cfg.runtime.resolve_message.return_value = _make_resolved_message()
        cfg.runtime.resolve_engine.return_value = "claude"
        cfg.runtime.format_context_line.return_value = None
    ...
    @pytest.mark.anyio()
    async def test_attach_referenced_files_called(self, sessions, chat_prefs):
        cfg = _make_cfg()
        cfg.runtime.resolve_message.return_value = _make_resolved_message()
        cfg.runtime.resolve_engine.return_value = "claude"
        cfg.runtime.format_context_line.return_value = None
...
class TestDispatchMessage:
    ...
    @pytest.mark.anyio()
    async def test_journal_and_ledger_passed_through(self, sessions, chat_prefs):
        cfg = _make_cfg()
        msg = _make_msg(text="hello")
        journal = AsyncMock()
        ledger = AsyncMock()
        ...
        assert call_kwargs["journal"] is journal
        assert call_kwargs["ledger"] is ledger
...
class TestDispatchRtCommand:
    ...
    async def test_completed_session_provides_continue_and_close(self, chat_prefs):
        cfg = _make_cfg()
        msg = _make_msg(text="!rt follow-up more", root_id="t1")
        send = AsyncMock()
    ...
    async def test_active_session_provides_close_only(self, chat_prefs):
        cfg = _make_cfg()
        msg = _make_msg(text="!rt close", root_id="t1")
        send = AsyncMock()
    ...
    @pytest.mark.anyio()
    async def test_no_root_id_no_callbacks(self, chat_prefs):
        cfg = _make_cfg()
    ...
    @pytest.mark.anyio()
    async def test_ambient_context_resolved(self, chat_prefs):
        ctx = MagicMock()
        ctx.project = "myproj"
        ctx.branch = "feat"
        ...
        cfg = _make_cfg()
        msg = _make_msg(text="!rt topic", root_id="t1")
        send = AsyncMock()
        ...
        with patch("tunapi.mattermost.loop.handle_rt", new_callable=AsyncMock):
            await _dispatch_rt_command(
                "close", msg, cfg, {}, chat_prefs, rt_store, send
            )
